...
public class MyPatch {
	...
	//define el tama�o de paso que se va a utilizar en el runge kutta 
	...
	public double calculateExposedToinfectedRate() {
		...
		if (this.temperaturePatch<15) {
			exposedToInfectedRate=0;
		}
		else {
			double patchIncubationPeriod = 0;
			if (15<this.temperaturePatch && this.temperaturePatch<21) {
				patchIncubationPeriod=RandomHelper.nextDoubleFromTo(10, 25);
			} else if (21<=this.temperaturePatch && this.temperaturePatch<26) {
				patchIncubationPeriod=RandomHelper.nextDoubleFromTo(7,10);
			} else if (26<=this.temperaturePatch && this.temperaturePatch<31) {
				patchIncubationPeriod=RandomHelper.nextDoubleFromTo(4, 7);
			} else {
				patchIncubationPeriod=4;
			}
